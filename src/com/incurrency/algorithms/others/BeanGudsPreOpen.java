    ...
    public void run() {
        ...
        for(BeanSymbol s:filteredFutures){
            ...
            if(Math.abs(percentchange)>beanG.getStandardDev().get(i)){
              ...
              Launch.algo.getParamTurtle().getOms().tes.fireOrderEvent(-1,-1,s, EnumOrderSide.SHORT, s.getMinsize(), Math.ceil(s.getClosePrice()*(1+percentchange) * 20) / 20, 0, "GUDS", 6, "MOC",EnumOrderIntent.Init,beanG.getMaxOrderDuration(),beanG.getDynamicOrderDuration(),beanG.getMaxSlippage());
