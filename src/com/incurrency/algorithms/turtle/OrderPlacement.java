    ...
    public void bidaskChanged(BidAskEvent event) {
        ...
        if (activeOrders.containsKey(id) && a.getParamTurtle().getAggression()) {
            ...
            if (tempOrderInfo.getExpireTime() - System.currentTimeMillis() < tempOrderInfo.getOrigEvent().getDynamicOrderDuration() * 60 * 1000) {
                ...
                double firstlimitprice=activeOrders.get(id).getOrigEvent().getFirstLimitPrice(); //first limit price is the price initally generated by the strategy.
                double maxslippage=activeOrders.get(id).getOrigEvent().getMaxSlippage();
                //bidaskChanged might have subsequently given a new limit price. this new value is in field limitprice.
                double bidprice = 0;
                double askprice = 0;
                ...
                Boolean placeorder = newlimitprice > 0 && (Math.abs(newlimitprice - limitprice) > tickSize ? Boolean.TRUE : Boolean.FALSE) && Math.abs(newlimitprice - firstlimitprice) <  maxslippage* firstlimitprice;
                if (placeorder) {
